...
public:
    ...
    void generateCPUCodeForCNode(ComputeNode* cNode)
    {
        ...
        } else if (cNode->getOpType() == AGGREGATE_EDGE || cNode->getOpType() == UPDATE_EDGE)
        {
            std::cerr << "CPU backend: kernel for op " << cNode->getOp()
                      << " (" << getKernelName(cNode) << ") is not implemented yet" << std::endl;
        }
    }
